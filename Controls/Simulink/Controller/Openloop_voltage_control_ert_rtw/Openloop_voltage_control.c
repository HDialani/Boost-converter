...
void Openloop_voltage_control_step0(void) /* Sample time: [0.001s, 0.0s] */
{
   ...
   */
  rtb_SignPreSat = rtb_Sum1 * (real_T)Openloop_voltage_control_B.SCIReceive[7] +
   ...
   */
  rtb_NotEqual = !rtb_RelationalOperator3;
  rtb_LogicalOperator2 = ((Openloop_voltage_control_B.Volt_Protection >
    Openloop_voltage_control_P.Constant13_Value) && rtb_NotEqual);
  ...
  /* S-Function (c28xsci_tx): '<S7>/SCI Transmit' */
  {
    ...
    scia_xmit((char*)
              &Openloop_voltage_control_B.TmpSignalConversionAtSCITransmi[0], 6,
              2);
  ...
  if (rtb_SignPreSat >
      Openloop_voltage_control_P.DiscretePIDController1_UpperSat) {
    rtb_SignPreSat -= Openloop_voltage_control_P.DiscretePIDController1_UpperSat;
  } else if (rtb_SignPreSat >=
             Openloop_voltage_control_P.DiscretePIDController1_LowerSat) {
    rtb_SignPreSat = 0.0;
  } else {
    rtb_SignPreSat -= Openloop_voltage_control_P.DiscretePIDController1_LowerSat;
  }
  ...
  if (rtIsNaN(rtb_SignPreSat)) {
    ...
    rtb_y1 = (int16_T)fmod(rtb_SignPreSat, 256.0);
  }
...
void Openloop_voltage_control_initialize(void)
{
  ...
  /* SystemInitialize for Outport: '<S111>/ ' incorporates:
   *  Inport: '<S111>/In'
   */
  ...
  /* SystemInitialize for Outport: '<S112>/ ' incorporates:
   *  Inport: '<S112>/In'
   */
